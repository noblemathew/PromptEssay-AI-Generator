...
class App(tk.Tk):
    ...
    def show_setup(self):
        ...
        tk.Label(page, text="Enter the candidate's details, pick a topic, and load the question set.",
        ...
        tk.Label(t, text="15 questions are loaded for the chosen topic, ordered from easy to hard.",
        ...
        self.counts.pack(fill="x", pady=(18, 0))
        self.count_labels = {}
        ...
        row.pack(fill="x")
        self.btn_load = Button(row, "Generate questions", self._load_questions, kind="ghost")
    ...
    def _load_questions(self):
        ...
        if not topic:
            messagebox.showinfo("Pick a topic", "Choose a topic before generating questions.")
            return
        ...
        self.data["questions"] = [dict(q) for q in QUESTIONS[topic]]
        self.data["responses"] = {}
        ...
        self._refresh_counts(self.data["questions"])
        self.btn_start.set_enabled(True)
        self.btn_load.set_text("Reload questions")
    ...
    def _generate(self):
        ...
        made = []
        try:
            p = os.path.join(folder, prefix + "_Assessment.docx")
            self._build_assessment().save(p)
            made.append(p)

            src = self.data.get("transcript_source")
            if src and os.path.isfile(src):
                p2 = os.path.join(folder, prefix + "_Transcript.docx")
                self._build_transcript(src).save(p2)
                made.append(p2)

            p3 = os.path.join(folder, prefix + "_Record.json")
    ...
    def _record(self):
        ...
        for i, q in enumerate(self.data["questions"]):
            r = self.data["responses"].get(i, {})
            rows.append({
                "no": i + 1, "qid": q["qid"], "level": q["level"],
                "question": q["question"], "expected": q["expected"],
                "notes": r.get("notes", ""),
                "score": 0 if r.get("skipped") else r.get("score"),
        ...
        return {
            ...
            "tool": "%s %s" % (APP_NAME, VERSION),
            "topic": self.data["topic"],
            "candidate": self.data["candidate"],
            "started": self.data["started"],
            "finished": self.data["finished"],
            "summary": {"average": round(s["avg"], 2), "percent": round(s["pct"], 1),
                        "attempted": s["attempted"], "skipped": s["skipped"],
                        "by_level": {k: (None if v is None else round(v, 2))
                                     for k, v in s["by_level"].items()}},
            "interviewer_rating": self.data["rating"],
            "recommendation": self.data["decision"],
    ...
    def _build_assessment(self):
        d = Docx()
        cand = self.data["candidate"]
        s = self.stats()
        iid = self.data["interview_id"]

        d.title("Interview assessment")
        d.para("%s technical assessment  \u00b7  %s" % (self.data["topic"], iid),
               color="00786C", bold=True, size=11)
        d.small("Generated %s by %s %s. Offline record \u2014 no data left this machine."
                % (datetime.now().strftime("%d %b %Y, %I:%M %p").replace(" 0", " "),
                   APP_NAME, VERSION))
        d.rule()

        d.table(["Field", "Detail", "Field", "Detail"], [
            ["Interview ID", iid, "Topic", self.data["topic"]],
            ["Candidate", cand.get("candidate", ""), "Candidate ID", cand.get("cid", "")],
            ["Role", cand.get("role", ""), "Grade", cand.get("grade", "")],
            ["Team", cand.get("team", ""), "Requisition ID", cand.get("req", "")],
            ["Date", cand.get("date", ""), "Time", cand.get("time", "")],
            ["Interviewer", cand.get("interviewer", ""), "Duration", self._duration()],
        ], widths=[1750, 3050, 1750, 3050])

        d.h1("Score summary")
        lvl = s["by_level"]
        d.table(["Measure", "Result"], [
            ["Questions asked", str(s["total"])],
            ["Answered", str(s["attempted"] - s["skipped"])],
            ["Skipped (counted as 0)", str(s["skipped"])],
            ["Average score", "%.1f out of 5" % s["avg"]],
            ["Score", "%d%%" % round(s["pct"])],
            ["Basic average", "\u2013" if lvl["Basic"] is None else "%.1f" % lvl["Basic"]],
            ["Medium average", "\u2013" if lvl["Medium"] is None else "%.1f" % lvl["Medium"]],
            ["Advanced average", "\u2013" if lvl["Advanced"] is None else "%.1f" % lvl["Advanced"]],
            ["Interviewer rating", "%s out of 5" % (self.data["rating"] or "\u2013")],
            ["Recommendation", self.data["decision"] or "Not recorded"],
        ], widths=[4200, 5400])

        d.h1("Scores at a glance")
        rows = []
        for i, q in enumerate(self.data["questions"]):
            r = self.data["responses"].get(i, {})
            if r.get("skipped"):
            ...
            q_text = q["question"]
            if len(q_text) > 88:
                q_text = q_text[:85].rstrip() + "\u2026"
            rows.append(["Q%d" % (i + 1), q["level"], q_text, mark])
        d.table(["#", "Level", "Question", "Score"], rows,
                widths=[700, 1300, 5900, 1700])

        d.h1("Question by question")
        for i, q in enumerate(self.data["questions"]):
            r = self.data["responses"].get(i, {})
            if r.get("skipped"):
                mark = "SKIPPED \u2014 candidate did not know  (0 / 5)"
                colour = "B4643C"
            elif r.get("score") is not None:
                mark = "SCORE  %d / 5" % r["score"]
                colour = "00786C"
            else:
                mark = "NOT SCORED"
                colour = "6A7480"
            d.h2("Q%d  \u00b7  %s  \u00b7  %s" % (i + 1, q["level"], q["qid"]))
            d.para(q["question"], bold=True, space_after=80)
            d.para("Expected answer: " + q["expected"], size=9.5, shade="F4F7F8")
            d.para("What the candidate said: " + (r.get("notes") or "\u2014 not recorded \u2014"),
                   size=9.5, space_after=60)
            d.para(mark, bold=True, color=colour, size=9.5)
            d.rule()

        d.h1("Interviewer assessment")
        d.para("Overall rating: %s out of 5" % (self.data["rating"] or "\u2013"), bold=True)
        d.para("Recommendation: %s" % (self.data["decision"] or "Not recorded"), bold=True)
        d.para("Comments", bold=True, space_after=60)
        d.para(self.data["comment"] or "\u2014 no comments recorded \u2014",
               shade="F4F7F8", size=10)
        d.para()
        d.para("Interviewer: %s" % cand.get("interviewer", ""), size=9.5)
        d.para("Signed off: ______________________        Date: ______________", size=9.5,
               color="6A7480")
        return d

    def _build_transcript(self, src):
        ...
        d = Docx()
        d.title("Interview transcript")
        d.para("%s  \u00b7  %s" % (self.data["interview_id"], self.data["topic"]),
               color="00786C", bold=True, size=11)
        d.small("Source file: %s  \u00b7  imported %s"
                % (os.path.basename(src), datetime.now().strftime("%d %b %Y, %I:%M %p")))
        d.rule()
        d.table(["Field", "Detail"], [
            ["Interview ID", self.data["interview_id"]],
            ["Topic", self.data["topic"]],
            ["Date", self.data["candidate"].get("date", "")],
            ["Speakers", "Interviewer / Candidate" if anon
                         else "%s / %s" % (self.data["candidate"].get("interviewer", ""),
                                           self.data["candidate"].get("candidate", ""))],
            ["Anonymised", "Yes \u2014 %d name references replaced" % hits if anon else "No"],
        ], widths=[2600, 7000])
        d.h1("Transcript")
        blank = 0
        for line in text.splitlines():
            ...
            if not line:
                ...
                if blank > 1:
                    continue
                d.para(space_after=60)
                continue
            blank = 0
            speaker = re.match(r"^\s*(Interviewer|Candidate)\s*[:\-\u2013]", line, re.IGNORECASE)
            d.para(line, bold=bool(speaker), size=10, space_after=60)
        return d
