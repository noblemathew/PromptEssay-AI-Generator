...
class App(tk.Tk):
    ...
    def _apply_bank(self, topic, picked, note):
        self.data["banks"][topic] = {"questions": picked, "note": note}
    ...
    def _render_question(self):
        ...
        inner = tk.Frame(exp, bg="#F4F7F8", padx=14, pady=11)
    ...
    def show_export(self):
        ...
        tk.Label(page, text="Generate the record", bg=CANVAS, fg=INK,
                 font=font(20, "bold", "display")).pack(anchor="w")
        tk.Label(page, text="Two files per candidate: a plain transcript for the agent, "
        ...
        o3, ac = card(page)
        o3.pack(fill="x", pady=(0, 18))
        tk.Label(ac, text="What goes in", bg=SURFACE, fg=INK,
                 font=font(12, "bold", "display")).pack(anchor="w", pady=(0, 10))
        self.ans_var = tk.IntVar(value=1 if self.data.get("include_answers", True) else 0)
        tk.Checkbutton(ac, text="Include what the candidate said \u2014 both files carry "
                                "the questions and the marks either way",
                       variable=self.ans_var, bg=SURFACE, fg=INK, font=font(9.5),
    ...
    def show_done(self, folder, files):
        ...
        tk.Label(c, text="Next candidate keeps the team, role, grade, requisition ID and "
                         "your name, and moves every topic on to the next set of "
                         "questions. A fresh interview ID is issued.",
