...
REPORTS = [
    {
        ...
        "sheets": [
            {
                "old_sheet": "Weekly Report",          
                "website_sheet": "",              
                "old_header_row": 7,                   
                "old_data_start_row": 8,                
...
def process_sheet(old_wb, website_wb, cfg, old_file):
    ...
    match_by = cfg.get("match_by", "name")

    if match_by == "position":
        # Copy column-by-column in order, ignoring header names entirely.
        old_first = cfg.get("old_first_col", 1) - 1   # to 0-based
        website_first = cfg.get("website_first_col", 1) - 1

        n_website_cols = max(len(r) for r in website_rows)
        n_old_cols = len(old_headers)

        col_map = {}
        for offset in range(n_website_cols - website_first):
            old_idx = old_first + offset
            website_idx = website_first + offset
            if old_idx >= n_old_cols:
                print(f"    NOTE: website has more columns than the old file - "
                      f"extra website columns beyond {n_old_cols} are ignored.")
                break
            col_map[old_idx] = website_idx

        print(f"    Positional mode: copying {len(col_map)} columns in order.")
        print("    Check the first output row carefully - positional mode "
              "assumes both files list columns in the SAME order.")
    else:
        col_map = {}
        unmatched = []
        ...
        if not col_map:
            raise ValueError(
                f"No columns matched between '{old_sheet}' and '{website_sheet}'. "
                "Header wording differs - set \"match_by\": \"position\" "
                "in this tab's config to copy columns in order instead.")
